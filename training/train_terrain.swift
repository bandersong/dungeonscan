// DungeonScan — CreateML HEX TERRAIN classifier trainer (Xcode 26 / CreateML API).
//
// Sibling to train.swift (the dungeon side). Trains a CoreML image classifier
// on the synthetic terrain dataset that gen_terrain.py produced, using an
// explicit stratified train/val split (dataset_terrain_split/{train,val}) so
// validation accuracy is measured on ~15% held-out data we control.
//
// Run:
//   swift train_terrain.swift
//
// Writes models/TerrainClassifier.mlmodel (falls back to .mlpackage on
...
let trainURL = URL(fileURLWithPath: env("DS_TERRAIN_TRAIN_DIR", "\(root)/training/dataset_terrain_split/train"))
let valURL   = URL(fileURLWithPath: env("DS_TERRAIN_VAL_DIR",   "\(root)/training/dataset_terrain_split/val"))

@inline(__always) func acc(_ classificationError: Double) -> Double {
    return 1.0 - classificationError
}

// Discover class labels from the train directory (sorted) — matches terrain_labels.json.
let labels: [String] = {
...
do {
    ...
    let mlmodelURL   = modelsDir.appendingPathComponent("TerrainClassifier.mlmodel")
    let mlpackageURL = modelsDir.appendingPathComponent("TerrainClassifier.mlpackage")
    let modelURL: URL
    do {
